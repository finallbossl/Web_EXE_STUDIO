...
export default function BookingPage() {
  ...
  const [depositOption, setDepositOption] = useState<string>("")
  ...
  // Mock data - trong thực tế sẽ fetch từ API dựa trên studioId/makeupId
  const providerData = {
  ...
  const services: Service[] = studioId
    ? [
        {
          id: "1",
  ...
  // Mock booked slots - trong thực tế sẽ fetch từ API
  const bookedSlots = [
    {
      date: "2024-01-25",
  ...
  const generateTimeSlots = (selectedDate: Date): TimeSlot[] => {
    ...
    for (let hour = 8; hour < 18; hour += 2) {
      ...
      if (bookedDay?.timeSlots.includes(timeRange)) {
        status = "booked"
      }

      // Mock maintenance slot
      if (hour === 16) {
        status = "maintenance"
      }
  ...
  const selectedTimeSlotData = selectedTimeSlot ? generateTimeSlots(date!).find((t) => t.id === selectedTimeSlot) : null
  ...
  const calculateDiscount = () => {
    return isPromoApplied ? calculateTotal() * 0.1 : 0 // 10% discount
  }

  const calculateDeposit = () => {
    if (!selectedDepositData) return 0
    return (calculateTotal() - calculateDiscount()) * (selectedDepositData.value / 100)
  }

  const calculateRemaining = () => {
    return calculateTotal() - calculateDiscount() - calculateDeposit()
  }
  ...
  const validateStep = (stepNumber: number): boolean => {
    ...
    switch (stepNumber) {
      ...
      case 3:
        if (!depositOption) {
          newErrors.deposit = "Vui lòng chọn hình thức đặt cọc"
        }
        if (!paymentMethod) {
  ...
  const handleApplyPromo = () => {
    if (promoCode.toUpperCase() === "NEWUSER10") {
      ...
      setErrors((prev) => ({ ...prev, promo: "Mã giảm giá không hợp lệ" }))
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.location.href = `/booking/confirmation?id=${Date.now()}`
    } catch (error) {
                      ...
                      >
                        ...
                        <div className="flex items-start justify-between">
                          ...
                          <div className="text-right ml-4">
                            <p className="font-semibold text-primary text-lg">{service.price.toLocaleString()}đ</p>
                          </div>
                        </div>
                      </div>
                    ))}
